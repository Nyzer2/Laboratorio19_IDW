document.getElementById('search-btn').addEventListener('click', function() {
    ...
    if (!input) {
        showError('Por favor, ingresa un nombre o ID válido.');
        return;
    }
    fetch(`https://pokeapi.co/api/v2/pokemon/${input}`)
        ...
        .then(data => {
            ...
            const stats = data.stats.map(stat => `${stat.stat.name.charAt(0).toUpperCase() + stat.stat.name.slice(1)}: ${stat.base_stat}`).join(', ');
